...
TS=$(date +%Y%m%d-%H%M%S)
OUT="profiles/app-${TS}.cpuprofile"

echo "[profile] building..."
npm run -s build

echo "[profile] starting app with --cpu-prof (output: $OUT)"
node \
  --cpu-prof \
  --cpu-prof-dir=profiles \
  --cpu-prof-name="$(basename "$OUT")" \
  dist/main.js &
APP_PID=$!

cleanup() {
  if ps -p "$APP_PID" >/dev/null 2>&1; then
    echo "[profile] stopping app (pid=$APP_PID) to flush profile..."
...
echo "[profile] done; profile saved to $OUT"
